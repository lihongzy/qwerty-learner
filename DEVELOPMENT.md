...
- `src/app`：应用入口、路由、布局、全局 provider、应用级 stores
- `src/pages`：按页面组织业务 UI、hooks、stores 与局部逻辑
- `src/shared`：可复用 stores、类型、资源、工具函数、数据库与基础 UI
...
```text
src/
├── app/                         # 应用入口、路由、布局、provider、应用级 stores
│   ├── app.tsx
│   ├── router/
│   ├── layout/
│   ├── providers/
...
## 3. 应用级状态（Zustand Stores）

项目的 Zustand stores 分三层：

- `src/app/stores`：应用级状态，例如主题和全局信息面板
- `src/shared/stores`：跨页面共用状态
- `src/pages/typing/stores`：Typing 页面专属偏好和声音配置

### 3.1 共享练习状态

位置：

- [src/shared/stores/practice-session.ts](src/shared/stores/practice-session.ts)
...
- `currentDictId`：当前选中的词典 ID，带本地持久化
- `selectCurrentDictInfo(currentDictId)`：由词典 ID 派生出的词典对象
- `currentChapter`：当前章节号，带本地持久化
- `reviewModeInfo`：复习模式配置，带本地持久化
- `reviewModeInfo.isReviewMode`：当前是否处于复习模式

### 3.2 共享显示偏好

位置：

- [src/shared/stores/preferences.ts](src/shared/stores/preferences.ts)
...
### 3.3 Typing 专属配置 stores

位置：

- [src/pages/typing/stores/preferences.ts](src/pages/typing/stores/preferences.ts)
- [src/pages/typing/stores/sound.ts](src/pages/typing/stores/sound.ts)
...
#### keySoundsConfig / hintSoundsConfig
...
- [src/shared/stores/persist.ts](src/shared/stores/persist.ts)

这个封装负责 Zustand stores 的本地配置读取、写入和函数式 setter 兼容：

- 如果存储值类型不匹配，回退到默认值
- 如果旧版本配置缺少字段，用默认值补齐
- setter 支持 `nextValue` 或 `(previousValue) => nextValue`
...
- [src/pages/typing/hooks/useWordList.ts](src/pages/typing/hooks/useWordList.ts)
...
1. 读取当前词典 `selectCurrentDictInfo(currentDictId)`
2. 读取当前章节 `currentChapter`
3. 读取复习模式 `reviewModeInfo`
...
1. 先判断它属于：
   - 应用级状态：放 `src/app/stores`
   - 共享状态：放 `src/shared/stores`
   - Typing 私有配置：放 `src/pages/typing/stores`
2. 如果需要持久化，优先复用 `src/shared/stores/persist.ts`
...
1. 当前代码库按 `src/app / src/pages / src/shared` 组织，新页面逻辑优先放入对应 `src/pages/*` 目录。
2. 文档中所有数据结构应以 `src/pages/typing/store/type.ts` 与 `src/shared/lib/db/record.ts` 为准。
3. 对持久化配置对象新增字段时，优先复用 `src/shared/stores/persist.ts`，避免旧 localStorage 数据结构不兼容。
