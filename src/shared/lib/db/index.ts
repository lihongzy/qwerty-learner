...
export function useSaveChapterRecord() {
  ...
  const isRevision = usePracticeSessionStore((state) => state.reviewModeInfo.isReviewMode);
  const dictID = usePracticeSessionStore((state) => state.currentDictId);
  ...
  const saveChapterRecord = useCallback(
    (typingState: ChapterRecordInput) => {
      ...
      const correctWordIndexs = userInputLogs
...
export function useSaveWordRecord() {
  ...
  const saveWordRecord = useCallback(
    async (
      { word, wrongCount, letterTimeArray, letterMistake }: SaveWordRecordInput,
      options?: SaveWordRecordOptions,
    ) => {
      const timing = [];
