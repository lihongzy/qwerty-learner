...
export interface IWordRecord {
  word: string
  timeStamp: number
  // 正常章节为 dickKey ,其他功能则为对应的类型
  dict: string
  // 用户可能是在 错题/其他类似组件中，进行的练习则为null，start from 0
  chapter: number | null
  //正确次数中输入每个字母的时间差，可以据此计算出总时间
  timing: number[]
  // 出错的次数
  wrongCount: number
  // 每个字母被错误输入成什么，index为字母的索引，数组为错误的e.key
...
export class WordRecord implements IWordRecord {
  ...
  get totalTime() {
    return this.timing.reduce((acc, cur) => acc + cur, 0)
  }
}
...
export interface IChapterRecord {
  ...
  //错误按键次数，输错一个字母即记录
  wrongCount: number
  //用户输入的单词总数，可能会使用循环等功能使输入总数大于20
  wordCount: number
  //一次打对未犯错的单词列表，可以和wordNumber对比得出出错的单词indexes
...
export class ChapterRecord implements IChapterRecord {
  ...
  constructor(dict: string, chapter: number | null, time: number, correctCount: number, wrongCount: number, wordCount: number, correctWordIndexs: number[], wordNumber: number, wordRecordIds: number[]) {
    this.dict = dict
    this.chapter = chapter
    this.timeStamp = getUTCUnixTimestamp()
    this.time = time
    ...
    this.correctWordIndexs = correctWordIndexs
  ...
  get wordAccuracy() {
    return Math.round((this.correctWordIndexs.length / this.wordNumber) * 100)
  }
}
...
export interface IReviewDictRecord {
  dict: string
  revisionIndex: number
  createTime: number
}

export class ReviewDictRecord implements IReviewDictRecord {
  dict: string
  revisionIndex: number
  createTime: number

  constructor(dict: string, revisionIndex: number) {
    this.dict = dict
...
export class RevisionDictRecord implements IRevisionDictRecord {
  ...
  constructor(dict: string, revisionIndex: number, createTime: number) {
    this.dict = dict
    this.revisionIndex = revisionIndex
