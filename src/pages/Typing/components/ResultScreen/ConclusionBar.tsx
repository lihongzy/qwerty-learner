...
const ICON_MAPPER: IconMapper[] = [
  {
    ...
    text: (mistakeCount: number) =>
      mistakeCount > 0 ? `Great work. Only ${mistakeCount} words were incorrect.` : 'Perfect run. No mistakes at all.',
  },
  {
    icon: IconHandThumbUp,
    className: 'text-indigo-600',
    text: () => 'A few small mistakes, but the overall result is solid.',
  },
  {
    icon: IconExclamationTriangle,
    className: 'text-indigo-600',
    text: () => 'Too many mistakes this time. Give it another try.',
  },
]
...
const ConclusionBar = ({ mistakeLevel, mistakeCount }: ConclusionBarProps) => {
  ...
  return (
    <div className="flex h-10 flex-row items-center">
      <Icon className={clsx(className, 'h-5 w-5')} />
      <span className="ml-2 inline-block align-middle text-sm font-medium leading-10 text-gray-700 sm:text-sm md:text-base">{text(mistakeCount)}</span>
    </div>
  )
}
